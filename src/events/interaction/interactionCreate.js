...
const execute = async (interaction) => {
    ...
    if (
        !MUSIC_BUTTONS.includes(interaction.customId) &&
        !interaction.customId.startsWith("music_queuepage_")
    )
        return;
        ...
        return interaction.reply({
            embeds: [
                new EmbedBuilder()
                    ...
                    .setDescription(
                        "❌ You must be in a voice channel to control the music.",
                    ),
            ],
    ...
    try {
        if (interaction.customId.startsWith("music_queuepage_")) {
            ...
            const { buildQueueEmbed, buildQueueButtons } =
                await import("../../utils/queueBuilder.js");
        ...
        switch (interaction.customId) {
                ...
                await interaction.reply({
                    embeds: [
                        new EmbedBuilder()
                            .setColor(config.embedColor)
                            .setDescription(
                                "⚠️ Are you sure you want to stop the music and clear the queue?",
                            ),
                    ],
            ...
            case "music_help": {
                const helpEmbed = new EmbedBuilder()
                    .setColor(config.embedColor)
                    .setTitle("🎶 Music Player Controls Help")
                    .setDescription(
                        `**Row 1:**\n` +
                            `⏮️ **Restart** : Restart the current track from the beginning.\n` +
                            `⏪ **Previous** : Go back to the previous track.\n` +
                            `▶️/⏸️ **Play/Pause** : Pause or resume the music.\n` +
                            `⏩ **Skip** : Skip to the next track in the queue.\n` +
                            `⏭️ **Last** : Jump to the very last track in the queue.\n\n` +
                            `**Row 2:**\n` +
                            `🔁 **Repeat** : Toggle Repeat mode.\n` +
                            `🔀 **Shuffle** : Toggle Shuffle mode.\n` +
                            `⏹️ **Stop** : Stop the music and clear the queue entirely.\n` +
                            `📑 **Queue** : Show or hide the upcoming tracks in the embed.\n` +
                            `📜 **Lyrics** : Show or hide the lyrics for the current track.\n\n` +
                            `**Row 3:**\n` +
                ...
                await interaction.reply({
                    embeds: [
                        new EmbedBuilder()
                            .setColor(config.embedColor)
                            .setDescription(
                                "⚠️ This feature will be implemented soon.",
                            ),
                    ],
                    flags: MessageFlags.Ephemeral,
                });
        ...
        if (interaction.customId.startsWith("music_queuepage_")) {
            ...
            await interaction.update({
                embeds: [buildQueueEmbed(player, page)],
                components:
                    totalPages > 1 ? [buildQueueButtons(player, page)] : [],
            });
        }
    } catch (error) {
